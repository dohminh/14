...
def add_cashflow():
    if request.method == 'POST':
        try:
            ...
            # Validate category based on transaction kind
            if kind == 'Income':
                valid_categories = [c.value for c in IncomeCategory]
                if category not in valid_categories:
                    flash('Invalid category for Income transaction', 'error')
                    return redirect(url_for('views.cashflows'))
            elif kind == 'Expense':
                valid_categories = [c.value for c in Category]
                if category not in valid_categories:
                    flash('Invalid category for Expense transaction', 'error')
                    return redirect(url_for('views.cashflows'))
            elif kind == 'Savings':
                # Get all saving goals for the user
                valid_categories = [goal.name for goal in current_user.saving_goals]
